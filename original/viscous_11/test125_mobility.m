function test125_mobility(diff,ci,T,n2,thermophoresis,distribution_type,T_distribution_spec)
...
mobility_type=2;
% mobility_type==1 Constant mobility throughout
% mobility_type==2 M=2-exp(a*(c-ci_ave)^2)
mobility_spec=[log(2)/(1-mean(ci))^2];
...
if freezing==1
    freezing_type=3;
    xlen=1;
    freezing_spec=[xlen/10^-11,1*10^-7];
    
    freezing_spec=[1*10^-7,-2.2*10^14];
    
    freezing_spec=10^-5;
    
    % if freezing_type==3, spec=concentration variance that freezes
...
[ne,nx,ny,n,nfour,ll,li,lu,bl,bi,bu,tl,ti,tu,rl,ri,ru,...
    ...
    coef_n2,wwTerms]=...
    setUpGlobal_2d_new(nex,ney,fc,alpha,xlen,ylen,...
...
if freezing==1
    ...
    [freezing_zeros,freezing_ones]=freezing_setUp(irow,icol,nx,ny);
...
if simulation_continuation==0
    ...
    if export_data==1
        sf=[nex;ney;dt;obs_t;diff;include_fluc;ci_fluc;nr_tol;...
            ...
            dt_down;dt_up;dt_min;dt_ideal;bypass_tol;n1];
        dlmwrite([output_path 'specification/spec_general'],sf);
...
%-----------------
%     co=dlmread([pwd '/Results/Run_17/concentration/iteration_130']);
...
% --- Reset residual vector and Jacobian matrix ---------------------------
            ...
            [terms_sf,terms_sj]=terms_2d_new(sj,ne,n1,chi,...
            ...
            sf=sf_2d_new(ne,nfour,weights,nx,ny,n,co,dt,dxdy,sj,...
                terms_sf,wTerms,viscosity);
            
            fprintf('Computing Jacobian matrix...\n')
            sj=sj_2d_new(...
                nnz_,irow,icol,ny,n,weights,dt,dxdy,...
                terms_sj,wwTerms,nworkers,viscosity);
...
% --- Apply boundary conditions -------------------------------------------
            ...
            if freezing==1
                ...
                fprintf('Freezing solution partially\n')
                
                [sf,sj]=freezing_apply(...
                    freezing_type,freezing_spec,sf,sj,freezing_zeros,freezing_ones,...
...
% --- Resolve divergence --------------------------------------------------
        ...
        if jk==-3
            disp('Entire solution has been frozen')
			break
        end
