...
%     xbinodal(1:1:nT)=linspace(c_min,xspinodal(nT),nT);
    xbinodal(nT:1:2*nT-1)=linspace(xbinodal(nT),c_max,nT);
    guess(1)=ybinodal(nT);
...
elseif k==nT-1
    ...
    c2_remaining=nT-k;
    xbinodal(1:1:c2_remaining+1)=...
        linspace(c_min,xbinodal(c2_remaining+1),c2_remaining+1);
    syms T
    chi_symbolic=get_chi(T,entropy,T_theta);
end

spacer=xbinodal(2*nT-1)-xbinodal(2*nT-2)
...
%============== Fin ====================================================
    disp('Checking out')
    guess(2)=xbinodal(i+1)/2
%     guess(2)=0.0001
    ybinodal(nT)
    ybinodal(nT-1)
    ybinodal(nT-2)
    disp('End chiking out')
%     guess(2)=0.999999
%     sol=vpasolve([dfreeE_dc1(c1,n1,n2,chi_symbolic)==dfreeE_dc1(c2,n1,n2,chi_symbolic),m==dfreeE_dc1(c1,n1,n2,chi_symbolic),m*c1+b==freeE(c1,n1,n2,chi_symbolic),m*c2+b==freeE(c2,n1,n2,chi_symbolic)],[T c1 m b],guess)
    disp('Symbolically solve')
    sol=solve([dfreeE_dc1(c1,n1,n2,chi_symbolic)==dfreeE_dc1(c2,n1,n2,chi_symbolic),m==dfreeE_dc1(c1,n1,n2,chi_symbolic),m*c1+b==freeE(c1,n1,n2,chi_symbolic),m*c2+b==freeE(c2,n1,n2,chi_symbolic)],[T c1 m b]);
    error('Just stop ehrererere')
    try
        disp('TRYIIIIIIIIIIIING')
        T
        c1
        c2
        m
...
%         [l sol.T sol.c2 sol.m sol.b nT-1]
        ...
        ybinodal(j)=sol.T;
        xbinodal(i)=sol.c1;
        disp('Jusdfdsaf stte hereeeeeeeeee')
    catch
        warning('sup')
        sol.T
        sol.c1
