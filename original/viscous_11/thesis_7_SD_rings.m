...
name1='300 0.3 0.3 1';
name2='7300 0.3 0.3 1';
name3='65400 0.3 0.3 1';
name4='116300 0.3 0.3 1';
...
%         fc_specific_iterations,nx,ny,xlen,ylen,ci_ave,sig_dig,f_resolution,picked_steps)
% %         dlmwrite([folder 'exporting_figures/FT/FT_processing_status'],1)
%     dlmwrite([folder 'exporting_figures/FT/fc_specific_iterations'],[fc_specific_iterations picked_steps])
% end
...
%     for iframe=1:1:3
    ...
    frame=iterations(isimul);

    c=dlmread([folder 'concentration/iteration_' num2str(frame)]);
    time=dlmread([folder 'time/iteration_' num2str(frame)]);
    c_nodal=extract_nodal_weights_2D(c,nx,ny);


    handle=subplot(4,2,(isimul-1)*2+1);
    ...
    box on
    if isimul==1
        title(sprintf('Concentration profile'),'FontWeight','Normal')
    end
...
%     return
    ...
    plotter=subplot(4,2,(isimul-1)*2+2);
    if ~exist([folder 'exporting_figures/FT'],'dir')
        mkdir([folder 'exporting_figures/FT'])
        mkdir([folder 'exporting_figures/FT/real_term'])
        mkdir([folder 'exporting_figures/FT/imaginary_term'])
    ...
    if dlmread([folder 'exporting_figures/FT/FT_processing_status'])==0
        ...
        fprintf('Computing fourier transform now\n')
...
% %         dlmwrite([folder 'exporting_figures/FT/FT_processing_status'],1)
        ...
        if ~exist([folder 'exporting_figures/FT/real_term/iteration_' num2str(frame)],'file')
%             return
            ...
            for xth=1:1:f_resolution
                for yth=1:1:f_resolution
                    ...
                    [real,imaginary]=FT_real_imaginary_2d(...
                        c_relative,f_y(yth),f_x(xth),ny,nx,twopi);

                    real_terms(xth,yth)=real;
                    imaginary_terms(xth,yth)=imaginary;
                end
            end
            
            dlmwrite([folder 'exporting_figures/FT/real_term/iteration_' num2str(frame)],real_terms,'precision',sig_dig)
            dlmwrite([folder 'exporting_figures/FT/imaginary_term/iteration_' num2str(frame)],imaginary_terms,'precision',sig_dig)
...
%         dlmwrite([folder 'exporting_figures/FT/fc_specific_iterations'],[fc_specific_iterations picked_steps])
    ...
    [real_terms,imaginary_terms,resolution]=load_FT_2d(folder,iterations(isimul));
...
%         c_nodal=extract_nodal_weights_2D(c,nx,ny);
    ...
    colormap(gca,'hsv')
    if isimul==1
        title(sprintf('Amplitude spectrum'),'FontWeight','Normal')
    end
