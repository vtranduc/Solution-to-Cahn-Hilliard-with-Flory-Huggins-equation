function test120(name,distribution_type,T_distribution_spec)
...
picked_steps=[1 15 22 25 40 1141]; %Make it so only 
% picked_steps=[1 15 40 45 68 178];
...
phases_on=0;
% %----Very long time-------
fourier_transform_on=0; %This transforms all iterations
fourier_transform_new_on=0; %This uses bult-in fftn
fourier_transform_specific_iteration_on=0; %This transforms only 6 specific iterations
% %-------------------
...
if FT_specific==1
    ...
    if exist([folder 'exporting_figures/FT/fc_specific_iterations'],'file')
        fc_spec=dlmread([folder 'exporting_figures/FT/fc_specific_iterations']);
        match=1;
        for step=1:1:6
            if fc_spec(step+1)~=picked_steps(step)
                match=0;
                break
            end
        end
        if match==1
            fc=fc_spec(1);
        elseif match==0
            error('Only fc for different steps could be determined. Re-compute fourier transform please')
        end
    else
        warning('The value of fc could not be determined!')
        fc=input('Insert the value of n2 manually. Please be correct (Perhaps 20?)\n');
    end
%     else
%         fc=20; %JUST ASSUMPTIONS
%     end
    %----------------------------------------------------------------------
    index=0;
    my_fig=figure(ifig);
