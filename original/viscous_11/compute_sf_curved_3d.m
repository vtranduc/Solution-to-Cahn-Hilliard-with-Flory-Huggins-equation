function sf=compute_sf_curved_3d(neight,conc_,conco_,nx,ny,nz,nex,ney,weights,dt,n1,n2,determinants,diffT,chi,wTerms)
...
if isscalar(diffT)
    parfor gbf=1:1:neight %PARFOR MUST BE FEASIBLE!!!
        sf(gbf)=compute_sfth_sf_3d(gbf,conc_,conco_,nx,ny,nz,nex,ney,weights,dt,chi,diffT,n1,n2,determinants,wTerms);
    end
else
    parfor gbf=1:1:neight %PARFOR MUST BE FEASIBLE!!!
        sf(gbf)=compute_sfth_sf_gradient_3d(gbf,conc_,conco_,nx,ny,nz,nex,ney,weights,dt,chi,diffT,n1,n2,determinants,wTerms);
    end
end

end

function solution=compute_sfth_sf_gradient_3d(sfth,conc_,conco_,nx,ny,nz,nex,ney,weights,dt,chi,diffT,n1,n2,determinants,wTerms)
            ...
            for iy=1:1:3
                for iz=1:1:3
                    solution=solution+w(ix)*w(iy)*w(iz)*determinants(elements(i_positional_element),ix,iy,iz)*(...
                        weights(elements(i_positional_element),orientation,type,1,ix,iy,iz)*cont(ix,iy,iz)-diffT(exth,ix)*weights(elements(i_positional_element),orientation,type,1,ix,iy,iz)*...
...
function solution=compute_sfth_sf_3d(sfth,conc_,conco_,nx,ny,nz,nex,ney,weights,dt,chi,diffT,n1,n2,determinants,wTerms)
    ...
    if elements(i_positional_element)~=0
        ...
        for ix=1:1:3
            for iy=1:1:3
                for iz=1:1:3
                    solution=solution+w(ix)*w(iy)*w(iz)*determinants(elements(i_positional_element),ix,iy,iz)*(...
                        weights(elements(i_positional_element),orientation,type,1,ix,iy,iz)*cont(ix,iy,iz)-diffT*weights(elements(i_positional_element),orientation,type,1,ix,iy,iz)*...
                        ((-1/(con(ix,iy,iz)^2*n1)+1/((1-con(ix,iy,iz))^2*n2))*...
                        (conc_(elements(i_positional_element),2,ix,iy,iz)^2+conc_(elements(i_positional_element),3,ix,iy,iz)^2+conc_(elements(i_positional_element),4,ix,iy,iz)^2)+...
