...
if shape==10
    [nx,ny,nz,n,nfour]=setUp_serendipity(nex,ney,nez);
    nxny=nx*ny;
else
...
elseif shape==10
    ...
    [gbfs1,gbfs2]=sj_mapper_serendipity(nx,ny,nz,nnz_);
    ...
    determinants=isoparametric_determinant_3d(...
        ne,nex,nx,ny,nexney,X,Y,Z,parallel_computing);
    wTerms=generate_wTerms_serendipity(ne,weights);
    
    %---------------------------
    [bc_gbfs_r,n_bc_nodes]=list_boundaries_radial_derivative(nx,ny,nz);
    
%     normals=get_normals_cube2sphere(nex,ney,nez,nx,ny,nz);
    
    rotation_mx=ders_sph2Cart(bc_gbfs_r,n_bc_nodes,X,Y,Z);
    inverse_mx=test72(bc_gbfs_r,n_bc_nodes,X,Y,Z);
    
    % inverse_mx POINTS TO DIRECTION OF NORMAL!!!
    ...
    for iii=1:1:3
        for jjj=1:1:3
            take(iii,jjj)=inverse_mx(test,iii,jjj);
            take2(iii,jjj)=rotation_mx(test,iii,jjj);
        end
    end
    take
    ...
    normal_nodal=test74(bc_gbfs_r,n_bc_nodes,X,Y,Z);
...
elseif shape==10
    co=generate_co_serendipity(ci,include_fluc,ci_fluc,n,nfour,nx,ny,nz);
end
...
% === Update concentration profile ========================================
    ...
    elseif shape==10
        ...
        if predict_with_acceleration==1
            cp=predict_c_with_acceleration_3d(c,co,coo,dt,dto,dtoo,...
                parallel_computing,nfour);
            cooo=coo;
        elseif predict_with_acceleration==0
            cp=predict_c_3d(c,co,dt,dto,parallel_computing,nfour);
        end
        %-------------------------
    end
...
% % %         c=rand(1,nfour);
% %         
%         cr=test71(c,bc_gbfs_r,n_bc_nodes,rotation_mx);
%         cor=test71(co,bc_gbfs_r,n_bc_nodes,rotation_mx);
        
%         return
%         
%         crr=test71(cr,bc_gbfs_r,n_bc_nodes,inverse_mx);
%         
%         sum(abs(c-crr))
...
% === Previous concentration (Only for parallel algorithm) ================
    ...
    elseif shape==10
        conco_=compute_conc_order_zero_serendipity(ne,co,weights,nexney,nex,nx,nxny);
    end
...
%             return
        ...
        elseif shape==10
            ...
            fprintf('Computing conc...\n')
            sj=compute_conc_serendipity(ne,c,weights,nex,nexney,nxny,nx);
            fprintf('Computing sf and sj terms...\n')
            [terms,sf,sj]=compute_terms_curved_3d(ne,sj,chi,n1,n2,nex);
            
            fprintf('Computing residual vector...\n')
            sf=test69(...
                nfour,sf,conco_,nx,ny,nz,nex,ney,weights,dt,determinants,...
                diffT,wTerms,terms);
            
            fprintf('Computing Jacobian matrix...\n')
            sj=test70(...
                gbfs1,gbfs2,nnz_,dt,weights,nex,ney,nx,ny,nz,...
                determinants,diffT,wTerms,terms,sj);
...
%         sj=sparse(gbfs1,gbfs2,sj);
        ...
        sf=[sf zeros(1,n_bc_nodes)];
        
%-----------------------------------------------
%         sj_=zeros(nfour,nfour);
%         for ijk=1:1:nnz_
%             sj_(gbfs1(ijk),gbfs2(ijk))=sj(ijk);
%         end
% %         bc_gbfs_r,n_bc_nodes,rotation_mx
%         sj__=sj_;
% %         sj_=zeros(nfour,nfour);
%         for row=1:1:nfour
%             for inode=1:1:n_bc_nodes
%                 for compt=1:1:3
%                     sj_(row,bc_gbfs_r(inode)+compt-1)=...
%                     sj__(row,bc_gbfs_r(inode))*rotation_mx(inode,1,compt)...
%                     +sj__(row,bc_gbfs_r(inode)+1)*rotation_mx(inode,2,compt)...
%                     +sj__(row,bc_gbfs_r(inode)+2)*rotation_mx(inode,3,compt);
%                 end
%             end
%         end
%         
%         
%         sj=sj_;
%         normals=test73(n_bc_nodes,bc_gbfs_r,X,Y,Z);
% 
%         sj_adder=zeros(n_bc_nodes,nfour);
%         for row=1:1:n_bc_nodes
%             sj_adder(row,bc_gbfs_r(row):1:bc_gbfs_r(row)+2)=normals(row,:);
%         end
% %         sj_=[sj_;sj_adder];
% %         sf=[sf,zeros(1,n_bc_nodes)];
%         sj=sj_;
...
%         test=1;
%         
%         take1=normal_nodal(test,:);
%         take2=c_(bc_gbfs_r(test):bc_gbfs_r(test)+2);
%         
%         dot(take1,take2)
%         
%         for ijk=1:1:nfour
%             if sj(nfour+test,ijk)~=0
%                 sj(nfour+test,ijk)
%                 ijk
%             end
%         end
...
%         test_node=50;
%         
%         take=c_(bc_gbfs_r(test_node):1:bc_gbfs_r(test_node)+2);
%         
%         size(bc_gbfs_r)
%         n_bc_nodes
%         bc_gbfs_r(test_node)
%         size(c_)
%         take2=sj_adder(n_bc_nodes,:);
%         take2
%         take
%         
%         normals(test_node,:);
%         
%         dot(take,normals(test_node,:))
%         
%         normals(test_node,:)
% %         inverse_mx(test_node,1,:)
%--------------------------------
...
% === Evaluate error ======================================================
        ...
        fprintf('Evaluating current iterations...\n')
        err=sqrt(sum(c_.^2.0))
