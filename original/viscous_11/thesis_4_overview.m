function thesis_4_overview

% General process of the simulation
% Shows many types of plots

clear
clc

%---Specify folder here---
name='10000 0.3 0.3 1';
frames=[59 167 1214];
%-------------------------
...
if length(T)==2 && T(1)~=T(2)
    error('Not coded for gradient case')
elseif length(T)==1 || T(1)==T(2)
    grad_T=0;
    coef_T=diff*T(1);
    chi=dimensionless_chi(T(1),entropy);
    Two_chi_n1=2*chi/n1;
end
nx=nex+1;ny=ney+1;
twopi=2*pi;
...
if dlmread([folder 'exporting_figures/FT/FT_processing_status'])==0
    ...
    export_fourier_transform_data_specific_2d(folder,nworkers,...
        fc,nx,ny,xlen,ylen,ci_ave,sig_dig,f_resolution,frames)
%         dlmwrite([folder 'exporting_figures/FT/FT_processing_status'],1)
end

if ~exist([folder 'exporting_figures/data_folder/time_domain.dat'],'file')
    error('Create view_simulation video with analysis file first!')
end
...
for iframe=1:1:3
    ...
    c=dlmread([folder 'concentration/iteration_' num2str(frames(iframe))]);
    time=dlmread([folder 'time/iteration_' num2str(frames(iframe))]);
    ...
    camlight
    title(sprintf('Concentration profile at\nt = %d',time),'FontWeight','Normal')
    
    
    handle=subplot(4,3,3+iframe);
    ...
    handle=subplot(4,3,6+iframe);
    [real_terms,imaginary_terms,resolution]=load_FT_2d(folder,frames(iframe));
    ...
    if mod(resolution,2)==0
        ...
        c=dlmread([c_folder num2str(frames(iframe))]);
        c_nodal=extract_nodal_weights_2D(c,nx,ny);
    ...
    colorbar('location','eastoutside');
    colormap(gca,'hsv')
    title(sprintf('Amplitude spectrum at\nt = %d',time),'FontWeight','Normal')
    pos=get(handle,'Position');
    set(handle,'Position',pos.*[1 1 1.3 0.8])
...
%     if ~exist([folder 'exporting_figures/data_folder/variance_time.dat'],'file')
%         error('Create the video with analysis file first, and obtain FT_circular.dat from there!')
%     end
...
if ~exist([folder 'exporting_figures/data_folder/FT_circular.dat'],'file')
    error('Create the video with analysis file first, and obtain FT_circular.dat from there!')
end
FT_circular=dlmread([folder 'exporting_figures/data_folder/FT_circular.dat']);
time_domain=dlmread([folder 'exporting_figures/data_folder/time_domain.dat']);
plot(time_domain,FT_circular,'k.')
xlabel('Time, t');ylabel('log(|| {\fontname{Lucida Calligraphy}F} ||^2)')
grid on
grid minor
ytickformat('%,.1f')
box on
axis([min(time_domain) max(time_domain) min(FT_circular) max(FT_circular)])
title(sprintf('Structure\nfactor'),'FontWeight','Normal')

subplot(4,3,12)

if ~exist([folder 'exporting_figures/data_folder/energy.dat'],'file')
...
%         close(my_fig)
    ...
    if ~exist([folder 'exporting_figures/data_folder'],'dir')
        mkdir([folder 'exporting_figures/data_folder'])
    end

    dlmwrite([folder 'exporting_figures/data_folder/energy.dat'],energy)
else
...
plot(time_domain,relative_energy,'k.')
xlabel('Time, t');ylabel('\DeltaE')
grid on
grid minor
ytickformat('%,.1f')
box on
axis([min(time_domain) max(time_domain) min(relative_energy) max(relative_energy)])
title(sprintf('Relative\nenergy'),'FontWeight','Normal')

%---------------------------------------------------

subplot(4,3,10);

if ~exist([folder 'exporting_figures/data_folder/variance.dat'],'file')
    variance=get_variance_c(...
        ci_ave,ne,ney,nx,ny,dx,dy,weights,c_folder,...
        allocation,extra,load_less,load_more,nworkers,...
        nIterations);
    dlmwrite([folder 'exporting_figures/data_folder/variance.dat'],variance)
else
    variance=dlmread([folder 'exporting_figures/data_folder/variance.dat']);
end
plot(time_domain,variance,'k.')

xlabel('x');ylabel('\sigma^2')
grid on;grid minor;
% pos=get(handle,'Position');
% pos
% pos(4)=0.1;
...
% name1='E1 DQ 500000 0.5 [0.39 0.41] 1 DT=0 d4s[0,0.25]  (i51 500000 0.5 0.39 1)';
% name2='F1 DQ 500000 0.5 [0.39 0.41] 1 DT=1000000 d4s[0,0.25]  (i51 500000 0.5 0.39 1)';
% name3='G1 DQ 500000 0.5 [0.39 0.41] 1 DT=10000000 d4s[0,0.25] (i51 500000 0.5 0.39 1)';
% name4='H1 500000 0.5 [0.39 0.41] 1 DT=100000000 d4s[0,0.25] (i51 500000 0.5 0.39 1)';
% 
% frames=[1 30 81
%     1 30 81
%     1 30 81
...
name1='A1 DQ 500000 0.5 [0.41 0.39] 1 DT=0 d10s[3,0] (i51 500000 0.5 0.39 1)';
name2='B1 DQ 500000 0.5 [0.41 0.39] 1 DT=1000000 d10s[3,0] (i51 500000 0.5 0.39 1)';
name3='C1 DQ 500000 0.5 [0.41 0.39] 1 DT=10000000 d10s[10,3] (i51 500000 0.5 0.39 1)';
name4='D1 DQ 500000 0.5 [0.41 0.39] 1 DT=100000000 d10s[3,0] (i51 500000 0.5 0.39 1)';
...
% name1='A2 TQ';
% name2='B2 TQ 500000 0.5 0.39 1 (DQ 500000 0.5 [0.41 0.39] 1 DT=1000000 d10s[3,0] (i51 500000 0.5 0.39 1))';
% name3='C2';
% name4='D2';
% 
% frames=[1 100 693
...
% name1='M1 500000 0.3 [0.36 0.34] 1 DT=0 d10s[3,0]';
% name2='M6 500000 0.3 [0.36 0.34] 1 DT=-100000 d10s[3,0]';
% name3='M7 500000 0.3 [0.36 0.34] 1 DT=-1000000 d10s[3,0] version 2';
% name4='M8 500000 0.3 [0.36 0.34] 1 DT=-10000000 d10s[3,0] version 2';
% 
% frames=[1 111 396
%     1 124 379
%     1 116 649
%     1 89 157];
% 
% DTs=[0,10^5,10^6,10^7];

%--------------------------------------Done

% name1='M2 500000 0.3 [0.36 0.34] 1 DT=100000 d10s[3,0]';
% name2='M3';
% name3='M4 500000 0.3 [0.36 0.34] 1 DT=10000000 d10s[3,0]';
% name4='M5 500000 0.3 [0.36 0.34] 1 DT=100000000 d10s[3,0]';
% 
% frames=[1 119 382
%     1 121 169
%     1 163 545
...
%     ci_ave=mean(ci);
    ...
    for icol=1:1:3
        ...
        c=dlmread([folder 'concentration/iteration_' num2str(frames(isimul,icol))]);
        time=dlmread([folder 'time/iteration_' num2str(frames(isimul,icol))]);
        c_nodal=extract_nodal_weights_2D(c,nx,ny);
        
        figure(1)
        subplot(4,3,(isimul-1)*3+icol);
        surf(x,y,c_nodal,'edgecolor','none')
        view(az, el);
        xlabel('x');ylabel('y');zlabel('c')
        grid on
        ...
        if icol==1
            str=sprintf('D_T=%2.1e',DTs(isimul));
            h1 = text(-0.6, 1.2,str);
            set(h1, 'rotation', 90);
        end
...
%         continue
        ...
        figure(2)
        handle=subplot(4,3,(isimul-1)*3+icol);
        contourf(x,y,c_nodal,'linestyle', 'none');
        colorbar('Location','eastoutside')
        colormap jet
        xlabel('x');ylabel('y')
        ...
        pos=get(handle,'Position');
%         axes('Position',pos.*[0.1 0.5 0.3 0.3])
%         contourf(x,y,c_nodal,[0 ci_ave],'linestyle', 'none');
        
        set(handle,'Position',pos.*[1 1 1.3 0.8])
        
% %         axes('Position',[.7 .7 .2 .2].*pos)
%         axes('Position',[pos(1)+0.8*pos(3) pos(2)+0.7*pos(4) 0.2*pos(3) 0.2*pos(4)])
% %         surf(x,y,c_nodal,'edgecolor','none')
%         view(az, el);
%         contourf(x,y,c_nodal,[0 ci_ave],'linestyle', 'none');
...
set(gca,'FontName',axis_font,'FontSize',font_size)
set(gcf,'PaperUnits','inches','PaperPosition',[0 0 ImageSizeX_profile_6_figs ImageSizeY_profile_6_figs])
print('-dpng', strcat(export_folder,'contour.png') , strcat('-r',num2str(dpi)))
close(fig2)
