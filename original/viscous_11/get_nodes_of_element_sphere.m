function nodes=get_nodes_of_element_sphere(e,ne,nex,ney,nexney,nx,ny,nz,n,nxny,n_eSurf,n_nSurf,nx_)
...
yth=mod_sp(esurf,ney);
zth=(esurf-yth)/ney+1;
nodes(2)=get_node_3d(nx,yth,zth,nx,ny);
nodes(1)=ny*(zth-1)+yth+n_nSurf(1)+n;
nodes(4)=nodes(2)+nx;
nodes(3)=nodes(1)+1;
nodes(6)=nodes(2)+nxny;
nodes(5)=nodes(1)+ny;
