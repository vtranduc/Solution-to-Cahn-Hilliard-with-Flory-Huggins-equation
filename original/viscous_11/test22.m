function [solution,con,cono,conx,cony,conxx,conyy,...
    phi,phixx,phiyy]=test22(chi,c_,co_,weights_,ilocal,diff,T,n1,n2,dx,dy,dt)
...
for ix=1:1:3
    for iy=1:1:3
        solution=solution+w(ix)*w(iy)*(...
            phi(ix,iy)*cont(ix,iy)-diff*T*phi(ix,iy)*...
            ((-1/(con(ix,iy)^2*n1)+1/((1-con(ix,iy))^2*n2))*...
            (conx(ix,iy)^2+cony(ix,iy)^2)+...
            (1/(con(ix,iy)*n1)+1/((1-con(ix,iy))*n2)-2*chi)*...
