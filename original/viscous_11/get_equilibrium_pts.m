function [s1,s2,e1,e2]=get_equilibrium_pts(...
    n1,n2,entropy,T,tol,nInitialGuessPts,...
...
s1=(-beta-sqrt(radicant))/(2*alpha);
s2=(-beta+sqrt(radicant))/(2*alpha);
...
chi=dimensionless_chi(T,entropy);
[e1,e2]=tangent_finder(...
    s1,s2,n1,n2,chi,tol,...
    guess_accuracy,nInitialGuessPts,...
    suppress_warning);

% [lower_pt,higher_pt]=tangent_finder(...
%     infl_left,infl_right,n1,n2,chi,tol,guess_accuracy,n)
...
% c1=xbinodal(nT-k);
% c2=xbinodal(nT+k);
[e1,e2]=refine_binodal(e1,e2,n1,n2,chi,tol);
